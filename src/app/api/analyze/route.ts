...
export async function POST(req: NextRequest): Promise<NextResponse<ApiResponse<AuditResponse>>> {
  ...
  try {
    ...
    const quotaCost =
      1 +
      (hasUploadedScreenshot ? 2 : 0) +
      (githubUrl ? 1 : 0) +
      (force ? 1 : 0);
    const quotaSubject = userId ? `analyze:user:${userId}` : `analyze:anon:${getClientIP(req)}`;

    const shortLimit = await checkRateLimitAsync(req, {
      identifier: `${quotaSubject}:minute`,
      windowMs: 60 * 1000,
      maxRequests: userId ? 8 : 3,
      cost: quotaCost,
    });
    if (!shortLimit.success) {
      throw new RateLimitError(shortLimit.resetIn);
    }

    const hourlyLimit = await checkRateLimitAsync(req, {
      identifier: `${quotaSubject}:hour`,
      windowMs: 60 * 60 * 1000,
      maxRequests: userId ? 30 : 8,
      cost: quotaCost,
    });
    if (!hourlyLimit.success) {
      throw new RateLimitError(hourlyLimit.resetIn);
    }
    ...
    try {
      ...
      // Langfuse: Create full trace and flush
      if (finalized.auditId && isObservabilityEnabled()) {
        const trace = createFullAuditTrace({
          auditId: finalized.auditId,
          url: validUrl,
          userId,
          provider: auditResult.provider || "unknown",
          modelId: auditResult.provider?.split(" ")[0],
          visionModelId: auditResult.visionModelId,
          codeModelId: auditResult.codeModelId,
          hasScreenshots: screenshots.length > 0,
          hasGithubCode: !!githubCodeText,
          inputTokens: auditResult.inputTokens ?? 0,
          outputTokens: auditResult.outputTokens ?? 0,
          totalTokens: auditResult.totalTokens ?? 0,
          estimatedCostUsd: 0,
          latencyMs: Date.now() - startTime,
          frontendScore: auditResult.frontendScore,
          backendScore: auditResult.backendScore,
          launchScore: auditResult.launchScore,
          verdict: auditResult.verdict,
          issuesFound: auditResult.issues?.length || 0,
          highSeverityCount: auditResult.issues?.filter(i => i.severity === "high").length || 0,
          mediumSeverityCount: auditResult.issues?.filter(i => i.severity === "medium").length || 0,
          lowSeverityCount: auditResult.issues?.filter(i => i.severity === "low").length || 0,
          analysisMode: auditResult.analysisMode,
          warning: auditResult.warning,
        });
        await flushAuditTrace(trace);
      }
      ...
      // Langfuse: Trace even on failure
      if (finalized.auditId && isObservabilityEnabled()) {
        const trace = createFullAuditTrace({
          auditId: finalized.auditId,
          url: validUrl,
          ...
          estimatedCostUsd: 0,
          latencyMs: Date.now() - startTime,
          launchScore: errorResult.launchScore,
